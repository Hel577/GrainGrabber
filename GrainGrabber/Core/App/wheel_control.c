...
#define rx 81
#define ry 147
const double MOTOR_R=50;           //这个是麦轮的半径，单位为毫米
...
void position_cal(float* motor_fai_, float* car_distance_ , uint8_t* if_minus)//注意这里是角速度而不是线速度
{
...
// x+ is left (mm)  y+ is forward(mm)  
//angle+ is clockwise(du)  v is speed(……circle per minute)
void move_to_target(float x_target, float y_target, float angle_target, int v)
{
    ...
    // 逆运动学计算电机目标角度
    position_cal(motor_fai, car_distance, if_minus);
...
//use speed to control the position  forward is 0 degree  clockwise is -   
//Counterclockwise is +     w_z  Counterclockwise is +
void set_speed(float angle_target, float speed_magnitude, float w_z)  // 增加w_z参数
{
    // 限制角度范围在 -180° 到 +180°
